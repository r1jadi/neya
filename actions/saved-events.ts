...
export async function toggleSaveVenue(formData: FormData): Promise<{ saved: boolean }> {
  ...
  const slug = String(formData.get("venue_slug") ?? "").trim();

  if (!user) {
    redirect(`/login?next=${encodeURIComponent(slug ? `/venues/${slug}` : "/events")}`);
  }
  if (!venueId) redirect("/events");
  ...
  if (existing) {
    ...
    await trackDiscoveryMetric("venue_save", { venueId });
  }
