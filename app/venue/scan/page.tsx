...
export const metadata: Metadata = {
  title: `Scan tickets · Venue portal · ${SITE.name}`,
};

type Props = { searchParams: Promise<{ ok?: string; error?: string; info?: string }> };

export default async function VenueScanPage({ searchParams }: Props) {
  const q = await searchParams;
  await requireVenueUser("/venue/scan");

  return (
    <main className="mx-auto w-full max-w-lg flex-1 px-4 py-10 sm:px-6">
      <h1 className="font-[family-name:var(--font-display)] text-2xl font-bold text-white">Door scan</h1>
      <p className="mt-2 text-sm text-white/55">Paste the QR payload from the buyer&apos;s dashboard after payment is confirmed.</p>
      ...
      <p className="mt-8 text-sm"><Link href="/venue" className="text-sky-300 hover:underline">Back to venue portal</Link></p>
    </main>
  );
}
