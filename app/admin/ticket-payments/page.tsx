...
type Order = { id: string; merchant_order_reference: string; payment_provider: string | null; payment_status: string; status: string; amount_cents: number | null; currency: string | null; created_at: string; profiles: { display_name: string | null } | { display_name: string | null }[] | null; tickets: TicketRelation | TicketRelation[] | null };
...
export default async function TicketPaymentsPage({ searchParams }: Props) {
  ...
  const [{ data: orders, error }, { data: attempts }, { data: refunds }, { data: events }] = await Promise.all([
    admin.from("ticket_orders").select("id, merchant_order_reference, payment_provider, payment_status, status, amount_cents, currency, created_at, profiles(display_name), tickets(tier_name, events(title))").order("created_at", { ascending: false }).limit(250),
