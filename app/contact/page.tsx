    ...
    <div className="flex min-h-screen flex-col bg-[var(--background)]">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-12 sm:px-6 sm:py-16">
        <div className="max-w-2xl">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-fuchsia-300">{t.contactPage.eyebrow}</p>
          <h1 className="mt-3 font-[family-name:var(--font-display)] text-4xl font-bold text-white sm:text-5xl">{t.contactPage.title}</h1>
          <p className="mt-4 text-base leading-relaxed text-white/60">{t.contactPage.description}</p>
        </div>
        <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 sm:p-7">
            <h2 className="text-xl font-semibold text-white">{t.contactPage.sendMessageHeading}</h2>
            <p className="mt-2 text-sm text-white/55">{t.contactPage.getBackSoon}</p>
            <div className="mt-6"><ContactForm /></div>
          </section>
          <aside className="h-fit rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 sm:p-7">
            <h2 className="text-xl font-semibold text-white">{t.contactPage.contactInformation}</h2>
