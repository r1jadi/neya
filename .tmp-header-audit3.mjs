...
const results = [];
const LOCALES_ARG = process.argv[2] ? process.argv[2].split(",") : ["en"];
...
for (const locale of LOCALES_ARG) {
  if (locale !== "en") {
    for (const width of [1280, 1340, 1366, 1440, 1536, 1600, 1728, 1920]) {
      await audit(width, { locale });
    }
  }
}
await browser.close();
