...
export function MyNightButton({ stop, variant = "icon", className }: MyNightButtonProps) {
  ...
  return (
    <button
      ...
      aria-pressed={added}
      aria-label={added ? "Remove from My Night" : "Add to My Night"}
      title={blocked ? "My Night is limited to 3 stops" : added ? "Remove from My Night" : "Add to My Night"}
      className={cn(
    ...
    >
      ...
      {variant === "default" ? (added ? "Added" : blocked ? "3 max" : "+ My Night") : null}
    </button>
  );
}
