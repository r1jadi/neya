  ...
  return (
    <div className="space-y-4">
      <form className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-sm">
        <select name="provider" defaultValue={filters.provider ?? ""} className="rounded-lg bg-black/30 px-2 py-1.5"><option value="">All providers</option><option value="raiaccept">RaiAccept</option></select>
      ...
      {filtered.map((row) => {
        ...
        return <article key={row.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="text-right text-xs text-white/65"><p>{row.provider ?? "unassigned"} · {row.paymentStatus} · attempt {row.attemptStatus ?? "none"}</p><p className="mt-1">{timestamp(row.updatedAt)}</p></div>
          </div>
          ...
          <div className="mt-3 grid gap-2 text-xs text-white/65 sm:grid-cols-3"><p>Original: <span className="text-white">{money(row.amountCents, row.currency)}</span></p><p>Successfully refunded: <span className="text-white">{money(row.refundedCents, row.currency)}</span></p><p>Remaining: <span className="text-white">{money(remaining, row.currency)}</span></p><p>RaiAccept order: {row.providerOrderId ?? "missing"}</p><p>Purchase transaction: {row.providerTransactionId ?? "missing"}</p><p>Created: {timestamp(row.createdAt)}</p></div>
