...
export function AddToCalendarButton({ event, className = "" }: { event: Event; className?: string }) {
  ...
  const location = [event.venue?.name, event.venue?.address].filter(Boolean).join(", "); if (location) google.searchParams.set("location", location); if (event.description) google.searchParams.set("details", event.description);
  return <div className={`relative ${className}`}><button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/15 px-3 py-2 text-sm font-medium text-white/75 transition hover:border-sky-400/40 hover:text-white"><CalendarPlus className="h-4 w-4" />Add to calendar<ChevronDown className="h-3.5 w-3.5" /></button>{open ? <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-white/10 bg-zinc-950 shadow-xl"><a href={google.toString()} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)} className="flex items-center gap-2 px-3 py-2.5 text-sm text-white/80 hover:bg-white/5"><CalendarPlus className="h-4 w-4 text-sky-300" />Google Calendar</a><button type="button" onClick={() => { downloadCalendar(); setOpen(false); }} className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm text-white/80 hover:bg-white/5"><Download className="h-4 w-4 text-sky-300" />Download ICS <span className="ml-auto text-xs text-white/40">Apple · Outlook</span></button></div> : null}</div>;
}
