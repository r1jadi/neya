...
function buildSuggestions(t: ReturnType<typeof useI18n>["t"]): {
  label: string;
  href: string;
  icon: React.ReactNode;
}[] {
  return [
    { label: t.search.exploreTonight, href: "/events?when=tonight", icon: <CalendarDays className="h-4 w-4" /> },
    { label: t.search.browseVenues, href: "/#venues", icon: <MapPin className="h-4 w-4" /> },
    { label: t.search.browseGuides, href: "/guides", icon: <Compass className="h-4 w-4" /> },
    { label: t.search.openTheMap, href: "/map", icon: <MapPin className="h-4 w-4" /> },
  ];
}
          ...
          >
            ...
            >
              ...
              <div className="max-h-[60vh] overflow-y-auto p-2">
                {!query.trim() ? (
                  <div className="p-3">
                    <p className="px-2 text-xs font-semibold uppercase tracking-wider text-white/40">{t.search.explore}</p>
                ...
                {!loading && error ? (
                  <div className="p-6 text-center">
                    <p className="text-sm text-white/70">{t.search.somethingWentWrong}</p>
                    <button
                      type="button"
                ...
                {!loading && !error && query.trim().length >= 2 && total === 0 ? (
                  <div className="px-4 py-10 text-center">
                    <p className="text-lg font-semibold text-white">
                      {t.search.nothingFound} “{query.trim()}”
                    </p>
                    <p className="mt-1 text-sm text-white/50">{t.search.nothingFoundHint}</p>
                ...
                {!loading && !error && total > 0 ? (
                  <div className="space-y-3 p-1">
                      ...
                      <section key={group}>
                        <div className="flex items-center justify-between px-2 pt-2">
                          <p className="text-xs font-semibold uppercase tracking-wider text-white/40">
                            {group === "events" ? t.search.events : group === "venues" ? t.search.venues : GROUP_LABELS[group]?.label ?? group}
                          </p>
                    ...
                    <p className="px-2 pb-1 pt-2 text-center text-[11px] text-white/30">
                      {total} {t.search.results} for “{query.trim()}”
                    </p>
                  </div>
                ) : null}
              </div>
