...
--
-- The admin form promises "Guests are limited to this number: ticket sales and
-- reservations are capped by it", but nothing in the booking path enforced it:
-- reserve_ticket_order() only checked the tier's own stock and
-- create_reservation() never rejected oversubscription. An admin could set
-- capacity 10 while 200 tickets were sellable.
--
-- This migration makes the effective capacity (the event's own value, or the
-- linked venue's when the event inherits) the cap for BOTH tickets and
-- reservations: committed seats = all sold + reserved tickets for the event +
-- party sizes of active reservations (pending / pending_payment / confirmed).
-- Booking attempts that would exceed the cap are rejected with
-- message = 'capacity' so the UI can show a precise error.

-- Effective capacity + committed seats for an event. Capacity resolution
-- mirrors the admin form/saveEvent: the event's own value wins, otherwise the
-- venue's, otherwise no cap. Venue-level reservations (event_id null) count
-- against the venue's capacity too.
...
-- Reserve capacity before creating a reservation: nobody above capacity can
-- book, and the count includes the new party size.
...
-- Enforce the same cap on ticket orders: the event's effective capacity
-- counts sold + reserved tickets of EVERY tier plus active reservations.
...
begin
  ...
  -- Event capacity gate (effective = event value ?? venue value). Held under
  -- the advisory lock so two tiers can't oversubscribe together.
