      ...
      <div className="mx-auto flex w-full min-w-0 max-w-6xl flex-col gap-12 px-4 sm:px-6 xl:flex-row xl:items-center">
        <div className="w-full min-w-0 max-w-2xl flex-1">
          ...
          >
            ...
            {t.hero.liveInPrishtina} · {cityDate}
          </motion.div>
          <motion.h1
          ...
          >
            <NeonButton asChild>
              <Link href="/events">{t.hero.openTodaysFeed}</Link>
            </NeonButton>
            <Link
              href="/#map"
          ...
          <dl className="mt-10 grid w-full grid-cols-3 gap-4 border-t border-white/10 pt-8 text-center sm:text-left">
            <div>
              <dt className="text-[11px] uppercase tracking-widest text-white/40">{t.hero.hereNow}</dt>
              <dd className="mt-1 text-2xl font-bold tabular-nums text-white">{hereNow.toLocaleString()}</dd>
            </div>
            <div>
              <dt className="text-[11px] uppercase tracking-widest text-white/40">{t.hero.tonight}</dt>
              <dd className="mt-1 text-2xl font-bold tabular-nums text-white">{tonight}</dd>
            </div>
            <div>
              <dt className="text-[11px] uppercase tracking-widest text-white/40">{t.hero.vibe}</dt>
            ...
            {hasSpotlight && spotlight ? (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 space-y-2 p-6">
                  <p className="text-xs font-semibold uppercase tracking-widest text-fuchsia-300">{t.hero.featured}</p>
                  <p className="text-2xl font-bold text-white">
                    {spotlight.venue?.name ?? spotlight.venue_name ?? t.hero.venueTba} · {spotlight.title}
                  </p>
              ...
              <div className="flex h-full flex-col items-center justify-center p-8 text-center">
                ...
                <p className="mt-6 text-lg font-semibold text-white/80">{t.hero.eventsComingSoon}</p>
                <p className="mt-2 text-sm text-white/50">{t.hero.cityPulse}</p>
              </div>
            )}
          </div>
        </motion.div>
